...
import {Header, Slider, Footer, Text, Heading, Container, CharachterCard} from './components/config';
...
function App() {
    const [charachters, setCharachters] = useState(CHARACTERS);

    const handleLikeClick = (id) => {
        const newArrCharachters = charachters.map(
            item => (item.id === id) ?
                { ...item, isLike: !item.isLike } :
                { ...item });
        setCharachters(newArrCharachters);
    };
